...
const startSSE = (message) => {
  return new Promise((resolve, reject) => {
    ...
    const apiUrl = `${import.meta.env.VITE_API_BASE_URL}/ai/eat_app/chat/sse?message=${encodeURIComponent(message)}&chatId=${chatId.value}`;
...
onMounted(() => {
  ...
  messages.value.push({
    ...
    sender: 'ai',
    content: '欢迎使用饮食推荐大师！请问您今天想吃什么类型的食物，有什么饮食偏好或健康需求吗？',
    timestamp: new Date().toISOString()
  });
  scrollToBottom();
});
